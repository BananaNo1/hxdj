...
public class UpdateBillFeeForm {
    ...
    @Pattern(regexp = "^[1-9]\\d*\\.\\d{1,2}$|^0\\.\\d{1,2}$|^[1-9]\\d*$", message = "totalFee内容不正确")
    ...
    @Pattern(regexp = "^[1-9]\\d*\\.\\d{1,2}$|^0\\.\\d{1,2}$|^[1-9]\\d*$", message = "realMileage内容不正确")
    @Schema(description = "代驾里程数")
    private String realMileage;

    @NotBlank(message = "returnMileage不能为空")
    @Pattern(regexp = "^[1-9]\\d*\\.\\d{1,2}$|^0\\.\\d{1,2}$|^[1-9]\\d*$", message = "returnMileage内容不正确")
    @Schema(description = "返程里程数")
