...
public class CosUtil {
    ...
    public HashMap uploadPublicFile(MultipartFile file, String path) throws IOException {
        ...
        //向存储桶中保存数据
        PutObjectRequest putObjectRequest = new PutObjectRequest(bucketPublic, path, file.getInputStream(), objectMetadata);
        ...
        if (List.of(".jpeg", ".jpg", ".png", ".gif", ".bmp").contains(fileType)) {
            ImageAuditingRequest imageAuditingRequest = new ImageAuditingRequest();
            imageAuditingRequest.setBucketName(bucketPublic);
            imageAuditingRequest.setDetectType("porn,terrorist,politics,ads");
            imageAuditingRequest.setObjectKey(path);
            ImageAuditingResponse imageAuditingResponse = client.imageAuditing(imageAuditingRequest);

            if (!imageAuditingResponse.getPornInfo().getHitFlag().equals("0") || !imageAuditingResponse.getTerroristInfo().getHitFlag().equals("0") || !imageAuditingResponse.getPoliticsInfo().getHitFlag().equals("0") || !imageAuditingResponse.getAdsInfo().getHitFlag().equals("0")) {
